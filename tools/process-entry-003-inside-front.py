"""Build Entry 003's inside-front companion as transparent artwork.

This mirrors the approved Entry 003 narrative-page extraction: preserve the
generated RGB source, remove its near-white field, reconstruct neutral
black/red ink, fit it inside the same safe area, and emit the exact
924 x 1534 RGBA canvas consumed by the shared journal paper system.
"""
...
ENTRY = ROOT / "assets" / "journal" / "003"
SOURCE = ENTRY / "source" / "inside-front-imagegen-rgb.png"
OUTPUT = ENTRY / "inside-front.png"
REPORT = ENTRY / "source" / "inside-front-processing.json"
...
def main() -> None:
    ...
    bbox = alpha.getbbox()
    if bbox is None:
        raise RuntimeError("Entry 003 inside-front extraction produced no artwork")
