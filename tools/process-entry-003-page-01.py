"""Build Entry 003 Page 1 as a transparent production artwork layer.

The image-generation source is intentionally preserved under
assets/journal/003/source. This processor removes its near-white RGB field,
...
def extract_ink(source: Image.Image) -> Image.Image:
    ...
    # The generated source has a very pale neutral field (roughly 242-253 RGB).
    # Ink signal is measured from the darkest channel, while red is retained by
    # a separate chroma signal so faint marker edges survive the matte removal.
...
def main() -> None:
    ...
    bbox = alpha.getbbox()
    if bbox is None:
        raise RuntimeError("Entry 003 Page 1 extraction produced no artwork")
