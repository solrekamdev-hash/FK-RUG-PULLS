"""Build Entry 005 Page 3 as a transparent production artwork layer.

The image-generation source is intentionally preserved under
assets/journal/005/source. This processor removes its near-white RGB field,
...
def extract_ink(source: Image.Image) -> Image.Image:
    ...
    # Mirror the approved Entry 005 Pages 1-2 extraction exactly: separate
    # dark-ink and red-chroma signals so marker edges survive matte removal.
...
def main() -> None:
    ...
    bbox = alpha.getbbox()
    if bbox is None:
        raise RuntimeError("Entry 005 Page 3 extraction produced no artwork")
