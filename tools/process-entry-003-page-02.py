"""Build Entry 003 Page 2 as a transparent production artwork layer.

This mirrors the approved Page 1 production extraction: preserve the generated
RGB source, remove its near-white field, reconstruct neutral black/red ink,
fit it inside the same safe area, and emit the exact 924 x 1534 RGBA canvas
consumed by the shared journal paper system.
"""
...
SOURCE = ENTRY / "source" / "page-02-imagegen-rgb.png"
OUTPUT = ENTRY / "artwork" / "page-02.png"
REPORT = ENTRY / "source" / "page-02-processing.json"
...
def main() -> None:
    ...
    bbox = alpha.getbbox()
    if bbox is None:
        raise RuntimeError("Entry 003 Page 2 extraction produced no artwork")
